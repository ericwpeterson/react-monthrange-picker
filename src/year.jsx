...
class YearBase extends React.Component {
	...
	changeYear(n) {
		...
		if (
			(n < 0 && year === this.props.restrictionRange.start.format('YYYY')) ||
	...
	selectMonth(e) {
		...
		const selectedMonth = parseInt(target.getAttribute('data-idx'), 10);
			...
			<div>
				<div className="head">
					<h4 className="title clearfix">
						<button className="btn btn-plain year-down pull-left" onClick={this.reduceYear}>
							<i className="fa fa-chevron-circle-left" />
						</button>
						{currYear}
						<button className="btn btn-plain year-up pull-right" onClick={this.incYear}>
							<i className="fa fa-chevron-circle-right" />
						</button>
					</h4>
				</div>
				<div className="months">
					<div className="clearfix">{months}</div>
				</div>
			</div>
		);
	}
